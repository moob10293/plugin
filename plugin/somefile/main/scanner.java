...
public class EatingEvent implements Listener{
    ...
    public void whenEat(PlayerItemConsumeEvent event){
        ...
        if (eatenitem.getType() == Material.ROTTEN_FLESH){//rotten flesh
            event.setCancelled(true);//no, I don't want to add hunger on top of hunger
            eatenitem.setType(Material.AIR);//gets rid of the food that you ate(possible bug, gets rid of all the food in the stack)
        ...
        else if (eatenitem.getType() == Material.CHICKEN){
            event.setCancelled(true);

            person.setFoodLevel(person.getFoodLevel()+2);//no change here either
            eatenitem.setType(Material.AIR);
